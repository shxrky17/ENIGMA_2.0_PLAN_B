...
const ResumeSetup: React.FC = () => {
    ...
    const startInterviewSession = async () => {
        ...
        // Start Python voice engine when user clicks Start Interview.
        // Non-blocking: interview still starts even if Python engine fails.
        try {
            await fetch('http://localhost:8000/voice/run-script', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    candidate_name: extracted.name || 'Candidate',
                    role: topic || 'Software Engineer',
                }),
            });
        } catch {
            console.warn('Could not start Python voice engine');
        }
        ...
        try {
            const response = await startInterview({
                ...
                generatedQuestions: extracted.generatedQuestions ?? [],
            });
            // Pass real first question data to InterviewRoom via URL
            const firstQ = response.firstQuestion;
            const params = new URLSearchParams({
                ...
                role: topic,
                total: String(response.totalQuestions),
                q: firstQ?.text || '',
                qSkill: firstQ?.skill || allSkills[0] || 'General',
                qDiff: firstQ?.difficulty || difficulty,
                qTopic: firstQ?.topic || topic,
            });
            navigate(`/interview/${response.sessionId}?${params.toString()}`);
        } catch (err) {
